...
type ChatRoomTitle struct {
	...
	RoomKey     string `gorm:"column:room_key;primaryKey;type:varchar(50);not null;comment:고정된 형식의 룸 키"`
	MyRoomTitle string `gorm:"column:my_room_title;type:varchar(200);not null;comment:내가 설정한 방 제목"`
	UpdateFlag  string `gorm:"column:update_flag;type:varchar(3);not null;comment:변경 유무 Y인 경우 나의 title로, N인 경우 생성시 title로"`
	UpdateDate  string `gorm:"column:update_date;type:varchar(20);not null;comment:수정 시간"`
}

func (ChatRoomTitle) TableName() string {

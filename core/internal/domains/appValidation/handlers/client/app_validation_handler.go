...
import (
	...
	clientUsecases "core/internal/domains/appValidation/usecases/client"
...
type AppValidationHandler struct {
	usecase clientUsecases.AppValidationUsecase
	sfg     *config.ServerConfig
}

func NewAppValidationHandler(sfg *config.ServerConfig, uc clientUsecases.AppValidationUsecase) *AppValidationHandler {
...
func (h *AppValidationHandler) ValidateApp(c *gin.Context) {
	...
	if header.Hash == "" || header.Device == "" || header.Uuid == "" {
		response.SendErrorResponse(c, commonConsts.BAD_REQUEST, commonConsts.ERROR, commonConsts.E_104, commonConsts.E_104_MSG)
		return
	}
	...
	if err := json.NewDecoder(c.Request.Body).Decode(&body); err != nil {
		response.SendErrorResponse(c, commonConsts.BAD_REQUEST, commonConsts.ERROR, commonConsts.E_103, commonConsts.E_103_MSG)
		return
	}
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			response.SendErrorResponse(c, commonConsts.BAD_REQUEST, commonConsts.FAIL, consts.CORE_F101, consts.CORE_F101_MSG)
		} else {
			response.SendErrorResponse(c, commonConsts.SERVER_ERROR, commonConsts.ERROR, commonConsts.E_500, commonConsts.E_500_MSG)
		}
		return
	}
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			response.SendErrorResponse(c, commonConsts.BAD_REQUEST, commonConsts.FAIL, consts.CORE_F102, consts.CORE_F102_MSG)
		} else {
			response.SendErrorResponse(c, commonConsts.SERVER_ERROR, commonConsts.ERROR, commonConsts.E_500, commonConsts.E_500_MSG)
		}
		return
	} else {
		response.SendSuccessResponse(c, resDto.Body)
	}
}

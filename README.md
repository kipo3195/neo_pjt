...
동시 메시지 전송 시 Race Condition이 발생할 수 있습니다.
...
- **Session Isolation**  
  각 WebSocket Connection을 독립적인 Goroutine으로 분리하여 세션 단위 처리 구조를 구성했습니다.
...
### Problem
실시간 메시징 환경에서 **읽지 않은 메시지 수(Unread Count)** 등 빈번한 이벤트가 발생하면,  
클라이언트로 매번 전송 시 **Network I/O 부담 증가** 및 **서버 상태 관리 비용**이 발생합니다.  
이를 효율적으로 처리하고, 동시에 **동시성 문제**를 최소화할 필요가 있었습니다.
...
- **Network I/O 최적화**
   - 이벤트를 배칭 처리하여 전송 횟수 감소

- **동시성 안전성 확보**
   - Worker Local Map으로 Lock-Free 상태 관리

- **확장성 향상**
  - 사용자 증가 시 Worker Sharding으로 부하 분산 가능
...
따라서 안전하게 종료하고 리소스를 정리할 수 있는 **Graceful Shutdown 프로세스**가 필요했습니다.
...
### Design
이 문제를 해결하기 위해 **NATS 기반 메시징 아키텍처**를 도입했습니다.
...
- **Request-Reply 패턴**
  - 채팅방 생성과 같이 **특정 노드에서만 처리해야 하는 요청**이나  
    **노드 상태 조회와 같은 동기 응답이 필요한 작업**을 처리하기 위해 사용
...
- **분산 환경 상태 동기화**
  - 이벤트 Publish를 통해 모든 Notificator 노드의 로컬 상태를 일관되게 유지
...
스토리지에 **사용되지 않는 파일(Orphan File)** 이 남아 **불필요한 스토리지 비용**이 발생할 수 있습니다.
...
- **스토리지 비용 최적화**
    - Batch 기반 Orphan File 정리를 통해 불필요한 파일 저장 비용 감소
